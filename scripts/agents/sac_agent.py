...
class SoftActorCritic(nn.Module):
    ...
    def q_backup_strategy(self, next_qs: torch.Tensor) -> torch.Tensor:
        ...
        if self.target_critic_backup_type == "doubleq":
            ...
            #print("Debugging: double q strategy is being used")
        elif self.target_critic_backup_type == "min":
            qmin, _ = torch.min(next_qs, dim=0)
            next_qs = qmin
            #print("Debugging: clipped q strategy is being used")
        else:
    ...
    def entropy(self, action_distribution: torch.distributions.Distribution):
        ...
        action = action_distribution.sample() #rsample을 써야되나 걍 sample만 써도 되나? back propagation안해서 ㄱㅊ을듯
    ...
    def actor_loss_reinforce(self, obs: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            q_values = self.critic(obs=obs_expand.reshape(-1, *obs.shape[1:]),
    ...
    def update(
        ...
        for _ in range(self.num_critic_updates):
            critic_info = self.update_critic(obs=observations, action=actions, reward=rewards, next_obs=next_observations, done=dones)
