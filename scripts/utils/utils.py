...
def sample_trajectory(
    env: VirtualDynamicsHandler, policy: Union[MLPPolicy, CNNPolicy], max_length: int, render: bool = False
) -> Dict[str, np.ndarray]:
    """Sample a rollout in the environment from a policy."""
    ob, _ = env.reset()
    obs, acs, rewards, next_obs, terminals, image_obs = [], [], [], [], [], []
...
def sample_trajectories(
    env: VirtualDynamicsHandler,
    policy: Union[MLPPolicy, CNNPolicy],
    min_timesteps_per_batch: int,
    max_length: int,
    render: bool = False,
) -> Tuple[List[Dict[str, np.ndarray]], int]:
...
def sample_n_trajectories(
    env: VirtualDynamicsHandler, policy: Union[MLPPolicy, CNNPolicy], ntraj: int, max_length: int, render: bool = False
):
    """Collect ntraj rollouts."""
    trajs = []
    for _ in range(ntraj):
