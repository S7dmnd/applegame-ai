...
class PERReplayBuffer(ReplayBuffer):
    def __init__(self, capacity=1000000, alpha=0.6, beta=0.4, beta_anneal_rate=1e-6):
    ...
    def insert(self, observation, action, reward, next_observation, done):
        ...
        max_prio = self.priorities.max() if self.size > 1 else 1.0
        self.priorities[(self.size - 1) % self.max_size] = max_prio
    ...
    def sample(self, batch_size):
        ...
        self.beta = min(1.0, self.beta + self.beta_anneal_rate)
        return batch

    def update_priorities(self, indices, priorities):
        for idx, prio in zip(indices, priorities):
            self.priorities[idx % self.max_size] = prio